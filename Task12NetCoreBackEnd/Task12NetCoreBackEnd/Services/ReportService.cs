...
namespace Task12NetCoreBackEnd.Services
{
    public class ReportService : IReport
    {
        ...
        {
            List<MoneyOperation> monthOperation = await _db.MoneyOperations.Where(o => o.Date >= startDate && o.Date <= endDate).Include(t => t.FinanceType).ToListAsync();
        ...
        public async Task<Report> GetByDateAsync(DateTime date)
        {
            ...
            List<MoneyOperation> dayOperations = await _db.MoneyOperations.Where(o => o.Date == date).Include(t => t.FinanceType).ToListAsync();
